...
WibGaussFitFunction::WibGaussFitFunction(double particleMeanMass,
                                         double minMass,
                                         double maxMass,
    ...
    WibFitFunction(minMass, maxMass)
{
    ...
    sigma = new RooRealVar("sigma", "sigma", gaussSigmaStart, gaussSigmaMin, gaussSigmaMax);
    ...
    gaussFunction = new RooGaussian("gauss", "signal", *mass, *mean, *sigma);
    polFunction = new RooPolynomial("background","background", *mass, bkgArgSet);

    totalIntensity = new RooAddPdf("total","total", RooArgList(*gaussFunction, *polFunction), RooArgList(*sigshare));
    data = new RooDataSet("data","data", RooArgSet(*mass, *initialWeight), RooFit::WeightVar("initialWeight"));
}



WibGaussFitFunction::~WibGaussFitFunction(){

    delete mean;
    delete sigma;
    delete a1;
    delete a2;
    delete gaussFunction;
    delete polFunction;
}



FitResult* WibGaussFitFunction::DoFitD(double eventMass, double eventMass2){
...
double WibGaussFitFunction::ReturnCurrentQValue(){

    RooArgSet invMassArgSet(*mass);

    double r = sigshare->getVal();
    double s = gaussFunction->getVal(&invMassArgSet) * r;
...
void WibGaussFitFunction::SaveFitToFile(std::string fileName){
